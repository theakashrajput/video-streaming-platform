...
import { refreshAccessTokenService, registerUserService, userLoginService, userLogoutService } from "../services/userAuth.service.js";
...
export const userRegister = asyncWrapper(async (req, res) => {
    ...
    const { accessToken, refreshToken, updatedUser } = await registerUserService({
        userName,
        email,
        password,
        fullName,
...
export const userLogin = asyncWrapper(async (req, res) => {
    ...
    const { accessToken, refreshToken, updatedUser } = await userLoginService({ userName, email, password });
    ...
    return res
        ...
        .cookie("refreshToken", refreshToken, cookieOptions)
        .json(
            new AppResponse(200, "User logged in successfully", updatedUser.toSafeObj())
        );
});
...
export const refreshAccessToken = asyncWrapper(async (req, res) => {
    ...
    const { newAccessToken, newRefreshToken } = await refreshAccessTokenService(incomingRefreshToken);
